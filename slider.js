$( document ).ready(function() {
    ...
    sliderImage(bigImages);

   function sliderImage(bigImages){
//big slider
...
//doun slider
       $.each(smallImages, function( k, v ) {
            ...
            $(".down_div").append(
                '<div class="small_image_position">'+
                     '<img class="plus_hover" src="image/'+v+'" alt="">'+

                     '<div class="plus_image"  alt="">'+
                        '<div class="circle_icon_plus"  alt="">'+'</div>'+
                     '</div>'+
                '</div>'
            );
        });
...
//big slider left icon
    ...
    $('body').on('click', '.circle_icon_plus', function(){
        parents_div = $(this).parent().parent();
        parents_div.siblings().attr('tabindex', -1);

        parents_div.prev().attr('tabindex',0);
        parents_div.prev().prev().attr('tabindex',0);

        parents_div.attr('tabindex',0);

        parents_div.next().attr('tabindex',0);
        parents_div.next().next().attr('tabindex',0);

        console.log($(this).parents('.slick-track').find('div'));
        var slick_div = $(this).parents('.slick-track').find('div');
        if(slick_div.hasClass('activediv')) {
            slick_div.removeClass('activediv', 'slick-current');
        }
        ...
        $('#bigImageDiv .image_position img').each(function(){
            ...
            if(big_img_name == down_click_image){
                console.log(big_img_name);
                console.log(down_click_image);
...
!function (a) {
    ...
    }, b.prototype.checkNavigable = function (a) {var c, d, b = this;if (c = b.getNavigableIndexes(), d = 0, a > c[c.length - 1])a = c[c.length - 1]; else for (var e in c) {if (a < c[e]) {a = d;break}d = c[e]}
        return a
    ...
    // avtomat poxuma nkar@ nerqevi
        ...
        c.destroy(!0), a.extend(c, c.initials, {currentSlide: d}), c.init(), b || c.changeSlide({
            data: {
                message: "index",
                index: d
            }
        ...
        $('.slick-track div').removeClass('activediv');
...
//Second and first sliders relation (when click down slider image b\this image show in big slider)

        var active_dives = $('.slick-track div[tabindex=0]:eq( 2 )');
        active_dives.addClass('activediv');

        var down_div_img_src = $('.activediv img').attr('src');

            $('#bigImageDiv .image_position img').each(function(){

// get small image name cut last 5 later and check to all big imsges
               var big_img_name_substr = $(this).attr('src').substring(0, $(this).attr('src').length - 5);
               var big_img_name = big_img_name_substr +'.jpg';

               if(big_img_name == down_div_img_src){
                   var big_image_div = $(this).parents('#bigImageDiv').find('div');
                   if(big_image_div.hasClass('active')) {
                       big_image_div.removeClass('active').fadeOut(200);
                   }
